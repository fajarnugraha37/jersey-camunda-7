...
Sentinel Enforcement Platform adalah project latihan enterprise untuk regulatory enforcement dan complex case management. State repo saat ini mencakup phase 0-5 sesuai roadmap utama: foundation, intake, authentication/authorization, case lifecycle, embedded Camunda workflow, dan evidence intake berbasis MinIO dengan presigned upload/finalize/download flow. Repo ini juga masih membawa workflow reconciliation tooling yang sebelumnya sudah ditambahkan sebagai hardening slice tambahan.
...
3. Jalankan `make up` untuk menyalakan PostgreSQL, MinIO, Keycloak, dan bootstrap bucket MinIO.
...
- Integration test dengan Testcontainers PostgreSQL + Keycloak + MinIO untuk:
