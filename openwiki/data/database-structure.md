...
The Sentinel Enforcement Platform uses **PostgreSQL** as its single primary database. Key scope decisions:
- **Single database, single schema** — All tables live in the `public` schema of one PostgreSQL database.
- **No read replicas** — All reads and writes go to the same database.
...
|---|---|---|---|
| `sentinel` | `public` | Application | Primary enforcement database |
| Camunda engine | `public` (`ACT_*` tables) | Camunda 7 | BPMN workflow runtime state |
...
## Keys Constraints and Identifiers

All tables use **UUID primary keys** generated by the application layer (not by the database). Sequence-based key generation is used only for the application-level `case_number` via `generate_case_number()`.

| Constraint Pattern | Tables | Details |
|---|---|---|
| `PK = id (UUID)` | All 23 tables except `case_number_sequence` | `NOT NULL`, generated by Java `UUID.randomUUID()` |
| `UNIQUE (case_id)` | `recommendation`, `decision`, `sanction_obligation`, `appeal` | Enforces 1:1 relationship with `case_record` |
| `UNIQUE (consumer_name, event_id)` | `inbox_event`, `notification` | Idempotent processing key |
| `UNIQUE (parent_case_id, child_case_id, relationship_type)` | `case_relationship` | Prevents duplicate relationship edges |
| `UNIQUE (active_case_id)` (DEFERRABLE) | `case_assignment` | At most one active assignment per case |
| `CHECK (version >= 0)` | All mutable tables | Optimistic concurrency guard |
| `CHECK (status IN (...))` | All state-machine tables | Enumerated state transitions |
| `CHECK (parent_case_id <> child_case_id)` | `case_relationship` | No self-referencing relationships |

## Indexes and Access Paths

All indexes are defined in Liquibase changelogs. See [consistency.md](consistency.md) for locking index usage.

| Index | Table | Columns | Purpose |
|---|---|---|---|
| `idx_report_jurisdiction_created_at` | `report` | `jurisdiction_code, created_at` | Filter by jurisdiction, sort by creation |
| `idx_report_jurisdiction_status_created_at` | `report` | `jurisdiction_code, status, created_at` | Cursor-paginated case list |
| `idx_case_record_jurisdiction_status_created_at` | `case_record` | `jurisdiction_code, status, created_at` | Cursor-paginated case list by jurisdiction+status |
| `idx_case_record_assignee_created_at` | `case_record` | `assignee_user_id, created_at` | My cases query |
| `idx_case_record_jurisdiction_unit_classification_created_at` | `case_record` | `jurisdiction_code, assigned_unit_id, classification, created_at` | Unit+clearance filtered lists |
| `idx_evidence_case_updated_at` | `evidence` | `case_id, updated_at` | Case evidence list |
| `idx_outbox_event_pending` | `outbox_event` | `status, available_at, occurred_at` | Outbox polling query |
| `idx_outbox_event_lease` | `outbox_event` | `lease_expires_at` | Lease cleanup |
| `idx_inbox_event_created_at` | `inbox_event` | `created_at` | Inbox processing order |
| `idx_notification_case_created_at` | `notification` | `case_id, created_at` | Case notifications list |
| `idx_workflow_instance_status` | `workflow_instance` | `status` | Workflow reconciliation query |
| `idx_case_assignment_active_case_lookup` | `case_assignment` | `active_case_id` | Active assignment lookup |
| `idx_case_relationship_parent` | `case_relationship` | `parent_case_id, relationship_type` | Parent-to-child navigation |
| `idx_case_relationship_child` | `case_relationship` | `child_case_id, relationship_type` | Child-to-parent navigation |
| `idx_maintenance_operation_run_name_requested_at` | `maintenance_operation_run` | `operation_name, requested_at` | Operations audit query |

## Sequences Identity and Key Generation

The Sentine platform does **not** use PostgreSQL `SEQUENCE` objects. Case numbering uses a custom sequence table:

**Table:** `case_number_sequence`
**Function:** `generate_case_number(jurisdiction_code, case_type, calendar_year)` — see [database-programmability.md](database-programmability.md)

## Partitions Retention and Archival

**Not Observed.** No table partitioning, data retention policies, or archival procedures are defined in the current Liquibase changelogs. Tables grow unboundedly. The `outbox_event` and `inbox_event` tables have status-based purge candidates (`PUBLISHED`, `processed`) but no automated cleanup is configured.

## Views Materialized Views Synonyms and Links

**Not Observed.** No PostgreSQL views, materialized views, synonyms, or database links (`dblink`) are defined in any changelog.

## Ownership and Sensitive Data

| Table | Owner | Sensitive Data | Notes |
|---|---|---|---|
| `report` | Application | Personal data (reporter name, description) | Reporter name is PII; description may contain personal data |
| `case_record` | Application | Case summaries, classification | `classification` column enforces access tiers (PUBLIC/CONFIDENTIAL/SECRET) |
| `evidence` | Application | Evidence content metadata | References MinIO object storage for actual file content |
| `audit_event` | Application | Actor, action, before/after summaries | Required for regulatory compliance; immutable |
| `sanction` / `sanction_obligation` | Application | Enforcement actions | May contain personal enforcement data |
| `case_relationship` | Application | Case linkage | Relationship reasoning may reference sensitive case details |

## Database Structure Catalog

| Object | Type | Owner | Purpose | Keys or Constraints | Relationships | Sensitive | Lifecycle or Retention | Evidence |
|---|---|---|---|---|---|---|---|---|
| `report` | Table | Application | Intake report submission | PK: `id` (UUID), CHECK on `status `IN ('SUBMITTED','TRIAGED')` | 1→1 `case_record` via `report_id` | Reporter name (PII) | No automated retention | `0001-foundation.yaml` |
| `case_record` | Table | Application | Enforcement case record | PK: `id` (UUID), UNIQUE on `case_number`, CHECK on `status` | 1→* evidence, assignment, history, audit | Case summaries, classification | No automated retention | `0002-case-management.yaml` |
| `case_number_sequence` | Table | Application | Case number counter | PK: `(jurisdiction_code, calendar_year)` | None | None | Persistent | `0002-case-management.yaml` |
| `case_assignment` | Table | Application | Case assignment history | PK: `id` (UUID), UNIQUE DEFERRABLE `active_case_id` | → `case_record` | Actor assignments | Historical | `0002-case-management.yaml`, `0009-advanced-persistence-assignment.yaml` |
| `case_status_history` | Table | Application | Status transition audit trail | PK: `id` (UUID) | → `case_record` | Transition history | Historical | `0002-case-management.yaml` |
| `audit_event` | Table | Application | Immutable audit log | PK: `event_id` (UUID) | → `case_record` | Actor, action, before/after | Immutable; no purge | `0002-case-management.yaml` |
| `workflow_instance` | Table | Application | Camunda process correlation | PK: `(case_id, workflow_type)`, UNIQUE `process_instance_id` | → `case_record` | Workflow state | No automated retention | `0003-workflow.yaml` |
| `evidence` | Table | Application | Evidence metadata | PK: `id` (UUID) | → `case_record` | Classification metadata | No automated retention | `0004-evidence.yaml` |
| `evidence_version` | Table | Application | Evidence version tracking | PK: `id` (UUID), UNIQUE `(evidence_id, version_number)` | → `evidence` | None | Historical | `0004-evidence.yaml` |
| `evidence_upload_session` | Table | Application | File upload session tracking | PK: `id` (UUID) | → `case_record`, → `evidence` | File metadata, SHA256 | Expires at `expires_at` | `0004-evidence.yaml` |
| `outbox_event` | Table | Application | Transactional outbox | PK: `event_id` (UUID), CHECK on `status` | None | Event payload | Purge candidate (PUBLISHED) | `0005-messaging.yaml` |
| `inbox_event` | Table | Application | Idempotent consumer inbox | PK: `id` (UUID), UNIQUE `(consumer_name, event_id)` | None | None | Purge candidate (processed) | `0005-messaging.yaml` |
| `notification` | Table | Application | User notifications | PK: `id` (UUID), UNIQUE `(consumer_name, event_id)` | → `case_record` (nullable) | Notification body | No automated retention | `0005-messaging.yaml` |
| `recommendation` | Table | Application | Case recommendation | PK: `id` (UUID), UNIQUE `case_id` | → `case_record` | Proposed sanctions | No automated retention | `0006-phase7-decision-appeal.yaml` |
| `recommendation_review` | Table | Application | Recommendation review | PK: `id` (UUID) | → `recommendation` | Review outcome | Historical | `0006-phase7-decision-appeal.yaml` |
| `decision` | Table | Application | Enforcement decision | PK: `id` (UUID), UNIQUE `case_id` | → `case_record`, → `recommendation` | Decision details | No automated retention | `0006-phase7-decision-appeal.yaml` |
| `decision_version` | Table | Application | Decision version history | PK: `id` (UUID) | → `decision` | Published versions | Historical | `0006-phase7-decision-appeal.yaml` |
| `sanction` | Table | Application | Sanction record | PK: `id` (UUID), UNIQUE `decision_id` | → `decision` | Sanction details | No automated retention | `0006-phase7-decision-appeal.yaml` |
| `sanction_obligation` | Table | Application | Obligations with due dates | PK: `id` (UUID) | → `sanction` | Obligation details | Status lifecycle (ACTIVE→OVERDUE→SATISFIED) | `0006-phase7-decision-appeal.yaml`, `0011-advanced-persistence-maintenance-operations.yaml` |
| `appeal` | Table | Application | Appeal record | PK: `id` (UUID), UNIQUE `decision_id` | → `case_record`, → `decision` | Appeal rationale | Status lifecycle (ACTIVE→DECIDED) | `0006-phase7-decision-appeal.yaml` |
| `appeal_decision` | Table | Application | Appeal outcome | PK: `id` (UUID), UNIQUE `appeal_id` | → `appeal` | Outcome | Immutable | `0006-phase7-decision-appeal.yaml` |
| `case_relationship` | Table | Application | Case hierarchy | PK: `id` (UUID), UNIQUE `(parent, child, type)`, CHECK no self-ref | → `case_record` (x2) | Relationship reasoning | No automated retention | `0010-advanced-persistence-case-relationships.yaml` |
| `maintenance_operation_run` | Table | Application | Operations audit log | PK: `id` (UUID), CHECK on `result_status` | None | None | Historical | `0011-advanced-persistence-maintenance-operations.yaml` |
| `case_number_sequence` | Sequence table | Application | Atomic counter for case numbers | PK: `(jurisdiction_code, calendar_year)` | None | None | Persistent | `0002-case-management.yaml` |

## Knowledge Gaps

1. **No retention or archival** — No table partitioning, TTL, or archival procedure is defined. Unbounded growth expected for `audit_event`, `outbox_event`, `inbox_event`, and `notification` tables.
2. **No views or materialized views** — No read-optimized or denormalized views exist.
3. **No read replica support** — The application does not configure or use read replicas; all reads go to the primary.
4. **`appeal_decision` FK gap** — The `appeal_decision` table references `appeal(id)` but the `appeal.decided_by_appeal_decision_id` field creates a potential circular reference if not properly managed at the application layer.

## Source References

1. **Liquibase Changelogs** — `sentinel-persistence/src/main/resources/db/changelog/releases/0001-foundation.yaml` through `0011-advanced-persistence-maintenance-operations.yaml`
2. **Entity-Relationship Diagram** — See [business-data.md](/openwiki/business/business-data.md) for domain-level entity relationships
5. **Camunda ACT_* tables undocumented** — The Camunda engine schema tables (`act_ru_execution`, `act_hi_procinst`, etc.) are managed externally and not documented here. They are referenced by reconciliation queries.
